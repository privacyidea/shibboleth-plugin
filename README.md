...
- **Cookie attributes.** The cookie is set `HttpOnly`, `Secure` and `SameSite=Strict`. Its lifetime is bounded by privacyIDEA's policy; `privacyidea.remember_me_days` only bounds the IdP-side cookie.
